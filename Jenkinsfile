        ...
        stage ('Push to Octopus') {
            steps {
                withCredentials([string(credentialsId: 'OctopusAPIKey', variable: 'APIKey')]) {
                    sh """
                        ${tool('Octo CLI')}octo push --package target/petclinic.3.0.${BUILD_NUMBER}.jar --replace-existing --server https://samples.octopus.app --apiKey ${APIKey} --space Spaces-203
